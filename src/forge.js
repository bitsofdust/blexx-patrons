...
function roundWingL(fx,fy,R,n,style,ink,gr){
  ...
  s+=`<path d="${edge}" fill="none" stroke="${gr}" stroke-width="3.5" stroke-linejoin="round" stroke-linecap="round"/>`;return s;
}
function pointWingL(fx,fy,R,style,ink,gr){
  ...
  s+=`<path d="${d}" fill="none" stroke="${gr}" stroke-width="3.5" stroke-linejoin="round"/>`;return s;
}
...
function card(p,finish,px,uid){
  ...
  s+=`<text x="120" y="301" text-anchor="middle" font-family="${ROUND}" font-size="19" font-weight="700" letter-spacing="0.3" fill="${labelCol}" stroke="${c.shade}" stroke-width="3.5" stroke-linejoin="round" paint-order="stroke">${p.name}</text>`;
