...
function derive(seed,forced){
  ...
  return{seed,house,flux,wingSize:sz[0],wingR,wingTex:TEX[Math.floor(r()*TEX.length)],ribCount:8+Math.floor(r()*6),
    fluxPick:FLUX[Math.floor(r()*FLUX.length)],mood:Math.floor(r()*4),blush:r()<0.55,
...
function shp(cx,cy,r,shape,fill,stroke,sw){
  ...
  if(shape==='triangle')return `<path d="${blobPath([[cx,cy-r],[cx+r,cy+r],[cx-r,cy+r]],r*0.38,r*0.09)}" fill="${fill}"${st}/>`;
  if(shape==='square')return `<path d="${blobPath([[cx-r,cy-r],[cx+r,cy-r],[cx+r,cy+r],[cx-r,cy+r]],r*0.32,r*0.06)}" fill="${fill}"${st}/>`;
...
function roundWingL(fx,fy,R,n,style,ink,gr){
  ...
  const k=Math.max(2.5,R*0.07);
  const edge=`M${pts[0][0].toFixed(1)},${pts[0][1].toFixed(1)}`+scallopEdge(fx,fy,pts,k);
  let s=`<path d="M${fx},${fy}L${pts[0][0].toFixed(1)},${pts[0][1].toFixed(1)}${scallopEdge(fx,fy,pts,k)}Z" fill="${ink}"/>`+ribsTo(fx,fy,pts,style,gr);
  s+=`<path d="${edge}" fill="none" stroke="${ink}" stroke-width="3" stroke-linejoin="round" stroke-linecap="round"/>`;return s;
}
function pointWingL(fx,fy,R,style,ink,gr){
  ...
  let s=`<path d="${d}" fill="${ink}"/>`;
  s+=ribsTo(fx,fy,[upper,PT(fx,fy,R*0.84,210),tip,PT(fx,fy,R*0.82,158),lower],style,gr);
  s+=`<path d="${d}" fill="none" stroke="${ink}" stroke-width="3" stroke-linejoin="round"/>`;return s;
}
...
function face(cx,hy,hr,shape,ink,bg,mood,blush){
  const eyY=hy+(shape==='triangle'?hr*0.24:0),ex=hr*0.4,erx=Math.max(3,hr*0.23),ery=Math.max(4,hr*0.29);
  let s='';
  if(blush){
    const bly=eyY+ery*0.85,blx=ex+erx+2;
    s+=`<ellipse cx="${(cx-blx).toFixed(1)}" cy="${bly.toFixed(1)}" rx="3.6" ry="2.1" fill="${bg}" opacity="0.5"/>`;
    s+=`<ellipse cx="${(cx+blx).toFixed(1)}" cy="${bly.toFixed(1)}" rx="3.6" ry="2.1" fill="${bg}" opacity="0.5"/>`;
  }
  s+=`<g class="eyes">`;
  for(const sgn of [-1,1]){
    const px=cx+sgn*ex,py=eyY+ery*0.22;
    s+=`<ellipse cx="${px.toFixed(1)}" cy="${eyY.toFixed(1)}" rx="${erx.toFixed(1)}" ry="${ery.toFixed(1)}" fill="${bg}"/>`;
    s+=`<circle cx="${px.toFixed(1)}" cy="${py.toFixed(1)}" r="${(erx*0.55).toFixed(1)}" fill="${ink}"/>`;
    s+=`<circle cx="${(px-erx*0.2).toFixed(1)}" cy="${(py-erx*0.25).toFixed(1)}" r="${(erx*0.2).toFixed(1)}" fill="${bg}"/>`;
  }
  s+=`</g>`;
  const my=eyY+ery+4;
  if(mood===1){
    s+=`<path d="M${(cx-6.5).toFixed(1)},${my.toFixed(1)} A6.5 5.5 0 0 0 ${(cx+6.5).toFixed(1)},${my.toFixed(1)}Z" fill="${bg}"/>`;
  }else if(mood===2){
    s+=`<circle cx="${cx}" cy="${(my+2).toFixed(1)}" r="2.6" fill="${bg}"/>`;
  }else if(mood===3){
    s+=`<path d="M${cx-8},${my.toFixed(1)} Q${cx-4},${(my+4.5).toFixed(1)} ${cx},${my.toFixed(1)} Q${cx+4},${(my+4.5).toFixed(1)} ${cx+8},${my.toFixed(1)}" fill="none" stroke="${bg}" stroke-width="2.2" stroke-linecap="round"/>`;
  }else{
    s+=`<path d="M${cx-7},${my.toFixed(1)} Q${cx},${(my+6).toFixed(1)} ${cx+7},${my.toFixed(1)}" fill="none" stroke="${bg}" stroke-width="2.5" stroke-linecap="round"/>`;
  }
  return s;
}
...
function drawFigure(p,cc,cfg){
  ...
  // chassis v2: bigger head, chubbier limbs, rounded mound instead of plinth
  const hy=97,hr=24,by=181,br=32,mndY=260;
  let wings='';
  if(p.wingR>0){
  ...
  const lyTop=by+br*0.3,lw=10,lx=br*0.4,lst=` stroke="${bg}" stroke-width="3" paint-order="stroke"`;
  let body='';
  body+=`<rect x="${(cx-lx-lw/2).toFixed(1)}" y="${lyTop.toFixed(1)}" width="${lw}" height="${(mndY-lyTop).toFixed(1)}" rx="${lw/2}" fill="${ink}"${lst}/>`;
  body+=`<rect x="${(cx+lx-lw/2).toFixed(1)}" y="${lyTop.toFixed(1)}" width="${lw}" height="${(mndY-lyTop).toFixed(1)}" rx="${lw/2}" fill="${ink}"${lst}/>`;
  body+=`<rect x="${cx-5.5}" y="${(hy+hr*0.35).toFixed(1)}" width="11" height="${(by-br*0.55-(hy+hr*0.35)).toFixed(1)}" rx="5" fill="${ink}"${lst}/>`;
  body+=shp(cx,by,br,cfg.shape,ink,bg,4);
  body+=shp(cx,hy,hr,cfg.shape,ink,bg,4);
  body+=topper(cx,hy,hr,cfg.topper,ink);
  body+=face(cx,hy,hr,cfg.shape,ink,bg,p.mood,p.blush);
  let s=`<g class="patron-idle">${wings}${body}</g>`;
  s+=`<ellipse cx="${cx}" cy="${mndY}" rx="48" ry="9" fill="${ink}"/>`;
  return s;
}
